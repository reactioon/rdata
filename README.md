# RDATA
RDATA is an Data Layer that aim to be lightweight, fast, distributed and scalable. 
...
RDATA is based on `LevelDB`, so we are talking about an KVS (Key-Value-Store) that store the data inside of an specific location using hash table as reference to the data, and avoid random access. the kvs concept don't have server-client to access the data, because it is standalone. So, we create our own server-client around of it. The data inside of an node can be accessed over network using HTTP for multiple requests without locking. LevelDB only grown in an vertical way, with RDATA, we grow horizontally with multiples databases shared in the same machine or over network.

## KVS

Our focus is not create an KVS, but an storage layer that can work as database to serve data over network and with an minimal install as possible.
...
`RDATA` have books concept. So, the whole flow of the RDATA is based on how an library works. 

First we have an `Collection`, and inside of an collection we have `Books`, and inside of `Books` we have `Documents`. Documents can be compared as an line of book, but in our case an line can store a lot of things, not only simple text, but structured data like numbers, documents, images, videos or only byte chars.

COLLECTION (aka, Database) -----> BOOKS (aka, TABLE) -----> DOCUMENTS (aka, RECORDS)

## Usage

RDATA is projected to be easy to install and use. all common operations can be made with an http request in the server API, passing the right arguments to desired method you will get an return of request.
...
* **Linear store**  
When the data changes, the order of an record do not change.
...
* **Dynamic variables**  
Support to work with an key-value dynamically increasing or decreasing their value. (like redis, increase/decrease an number)

RDATA has some other useful features like Logs, REST API, Connection Pool, ACL (Whitelist) and much more.

## Install

The install an server node is easy, see the full command below:
...
When the installer is working, the folder base will be created and an copy of this file will be saved on desired location. Inside of location an file of settings called `rdata.yaml` will be created, your can change nodes config inside of it.

**Notes:**

**(1):** If you are running an RDATA node with Linux that have `systemctl` installed, you can create the service automatically. This will enable the server start with the command `systemctl start rdata`.  

**(2):** `Location` is where your RDATA version will be saved, inside of it has an folder called data, inside of this folder you have the whole collection data.

## Run

The start an server node run the RDATA with `-run-server` flag, see the full command below:

```sh
./rdata -run-server
```

**Note:** When it's running the server-client will be started an can be open on browser. Check the list of endpoints below.

### Postman

To easy explore the features inside of RDATA, an collection of methods are available to postman. So, you only import it and it's done, you can explore all options.
...
## List
List is an easy way to get an list of documents inside of an book, and the data returned will stay in the order that they are added. So when the data changes the list still the same.
...
Inside of reactioon the data are temporary or persistent, to work with these scenarios inside an   collection of books has an group of documents called `variables` that can be used as an temporary/persistent storage. The content of an variable can be accessed from multiples locations without lacking. So, to improve the performance of your solution, the variables can be used to be an fast way to distribute data inside of an application like config settings.
...
RDATA has resources to create an way search documents very fast, but the search only works with indexed fields for structured documents like JSON.

#### Documents / Search
**Endpoint (GET)**: /rdata/`{collection}`/`{book}`/_docs/_search?field=`{field}`&condition=`{condition}`&value=`{value}`&limit=`{limit}`&meta=`{meta}`

Search endpoint has an aditional number of fields to determine what is the context of the search. check the details below:
...
Most of solutions don't need high level of security, in most of cases, they prevent data using an Firewall. The default install makes the security disabled but only enable the access only from the machine installed (using ipv4).

If you want control the security of your data, you can setup ACL (Access Control Layer) to add hosts that can call with an collection of data. to change this, check the `SERVER_WHITELIST` inside of `rdata.yaml`.

## Version

This an Release Candidate (RC) version, so this mean that this tool will be used in the whole ecossystem of reactioon.
...
RDATA has an set of arguments to help developers test the tool before starting their project. So, the developer can check performance and how it works.
...
With the command above we will populate the database with 10 records, to do stress test we can use an  large amount os records.

**Loop tests**

Looped test is an way to do stress test of an node without intervention. So, you can use some actions to see how it will perform after a while. See the options below:
...
```
This test will perform read of an variable each times.
...
**5. Insert X documents inside of an book**
...
* **Blockchain L1/L2/...**  
The blockchain is an sequence of data blocks connecting each block using an signature, the most common usage of signature is an hash sha256/512. The same structure of an blockchain can be created inside of RDATA, on this is case RDATA it's an KVS (Key-Value-Store) that can scale horizontally and can be accessed over network, with shared variables and indexes support.

* **Standalone Apps**  
Many online solutions needs their own database installed locally to increase performance, on browser most of softwares can use indexedb to save data locally, but when works with multiple computers, an standalone database for each computer is needed. RDATA can be shipped inside of any software to create an data layer that can be accessed over network.
...
* **Web App**  
Web apps needs to be flexible, fast, secure and scalable. RDATA can be used to create an network to serve any type of data, to show as you want inside of your app using HTML+JAVASCRIPT. Your app will have their own database (installed locally).

* **DApps**  
DApps are solutions that use an smart contract inside of an blockchain as reference to execute an process, many of types processes can be executed with smart contracts. The problem with smart contracts to DApps are the cost of network with high usage, but the cost can be solved with a Layer 2 of an blockchain. It's mean the DApp will store their own data or make their own blockchain. So, you can use RDATA to store your own data with an network and send to the blockchain only an signature of your content.

* **Data Science**  
Data science needs to work with large amount of data to make predictions, RDATA can help scientists work with large amount of data inside of an distributed environment over an network public/private.

## License

The usage of the RDATA is restricted to reactioon users, so the user needs an reactioon account and RTN to use it inside of their project.
...
RDATA is in development but can be used in development/production, actually the public version is only to test performance and resources comsumption, not an version ready to production. But, after a lot of tests inside of reactioon, RDATA can be the solution to all of our data structure. 
